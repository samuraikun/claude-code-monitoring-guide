...
Claude Code Hooks (lifecycle-logger-hook, Go バイナリ)
...
セッション単位でどのスキル・コマンド・エージェント・ツールが使用されたかを追跡するダッシュボード。DuckDB + Claude Code Hooks（Go バイナリ）で実現。

**使い方**:
1. 全セッションの概要統計と使用内訳を確認
2. Session List で `session_id` をクリックしてドリルダウン
3. Session Event Timeline でセッション内の全ライフサイクルイベント（全ツール使用含む）を時系列で確認
...
| Total Tool Uses / Unique Tools | Stat | PostToolUse hook による全ツール統計（Read, Edit, Bash, Grep 等） |
| Top Tools by Usage | Bar chart | ツール別の使用回数ランキング |
| Session List | Table | 全セッション一覧（モデル・プロンプト数・スキル/エージェント/コマンド/ツール数）。session_id クリックでドリルダウン |
| Session Event Timeline | Table | セッション内の全ライフサイクルイベント（tool_use イベント含む）を時系列表示 |
...
全セッション横断でのスキル・コマンド・エージェント・ツール使用トレンドを追跡するダッシュボード。モデル別・プロジェクト別・ツール別の分析も可能。
...
| Daily Tool Usage | Time series | ツール使用回数の日別推移 |
| Tool Distribution | Bar chart | 全セッション横断のツール使用ランキング（Read, Edit, Bash, Grep 等） |
...
├── hooks/
│   ├── lifecycle-logger/           # Hook バイナリの Go モジュール
│   │   ├── main.go                 # イベントハンドラ（PostToolUse、Stop 時トークン集計等）
│   │   ├── truncate.go             # tool_input/tool_response の 10KB トランケーション
│   │   ├── transcript.go           # セッション全体のトークン使用量を transcript から解析
│   │   └── *_test.go               # ユニットテスト（23 件）
│   ├── lifecycle-logger-hook       # コンパイル済み Go バイナリ（gitignore、go build で生成）
│   └── lifecycle-logger.sh         # 旧 Bash Hook スクリプト（参照なし、互換用に残置）
